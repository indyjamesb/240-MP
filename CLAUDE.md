...
- **A virtual channel is a timeline that is built, not played live.** Editing a pool only writes `channels.json`; nothing airs differently until that channel is rebuilt, which is why every editing screen carries a BUILD CHANNEL row. Local files, Plex, Jellyfin and Emby are all browsed and stored in one shape — ask the backend what a source supports (e.g. `source_supports_playlists()`) rather than hardcoding it in a view. Exclusions live in the channel's source block, not on the entry. Where a channel had got to is stored as refs (`marks`/`mark`), never as offsets, so editing a pool cannot move the other shows' places. A channel's `kind` is `tv` or `movies`; a movie channel airs films as programmes, has no slots and no grid, and takes its order from whether its films come from a playlist or a selection. (Details: [ARCHITECTURE.md → Virtual Channels](ARCHITECTURE.md#virtual-channels-scheduled-tv).)
